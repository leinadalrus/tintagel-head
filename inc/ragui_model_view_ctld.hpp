...
template <typename Datatype> class ObserverSoA {
public:
...
class ModelViewController : public ModelViewCommand {
  ModelViewCommand model_view_command =
      ModelViewCommand{}; // need a copy constructor
public:
  void execute_command_model_view() { this->handle_controller_model_view(); }
...
public:
  ...
  const bool
  update_onevent_usesignal(const ViewComponentEntity &entity_component,
                           ViewObservedEvents observed_events);
};
...
public:
  void add_observer(ViewComponentObserver *mvc_observer);
  // constexpr ObserverSoA<ViewComponentObserver>
  // add_observer(ViewComponentObserver *mvc_observer);
  void remove_observer(ViewComponentObserver *mvc_observer);
  // constexpr ObserverSoA<ViewComponentObserver>
  // remove_observer(ViewComponentObserver *mvc_observer);
};
